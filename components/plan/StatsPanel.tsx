...
function Row({ label, value, swatch, accent, sub, green }: {
  label: string; value: string | number;
  swatch?: string; accent?: boolean; sub?: boolean; green?: boolean;
}) {
  return (
    <div className={`flex items-center justify-between py-1.5 text-sm ${accent ? "font-semibold text-forest" : sub ? "text-muted" : "text-ink"}`}>
    ...
    <div className="space-y-2">
      ...
      {!tooManyTiles && (
        <div className="divide-y divide-line rounded border border-line bg-paper px-3">
          ...
          {stats.savedTiles > 0 && (
            <Row label="Reused offcuts" value={`−${stats.savedTiles} tile${stats.savedTiles > 1 ? "s" : ""}`} sub green />
          )}
          <Row label="Physical cut tiles" value={stats.physicalCutTiles} swatch="#e8a838" />
