...
import { plannerReducer, createInitialState } from "./lib/plannerReducer";
import { exportPdf } from "./lib/exportPdf";
import type { PlannerConfig } from "./lib/config/types";
import { indoorConfig } from "./lib/config/indoorConfig";
import { outdoorConfig } from "./lib/config/outdoorConfig";
import { TileGrid } from "./zones/TileGrid";
import { TileGridBackground } from "./canvas/TileGridBackground";
import { PatioPolygon } from "./zones/PatioPolygon";
import { TileTooltip } from "./canvas/TileTooltip";
import { ShapeEditor } from "./canvas/ShapeEditor";
import { PlannerSidebar } from "./sidebar/PlannerSidebar";
import { Toast } from "@/components/ui/Toast";
import { Button } from "@/components/ui/button";
import { IconButton } from "@/components/ui/icon-button";
import { resolveTileSize, pixelToWorld } from "./lib/geometry";
import { TILE_PRESETS } from "./lib/constants";
import type { Vertex, PlanType, PlannerAction } from "./lib/types";
import type { PlanExport } from "./lib/schema";
import { PlanExportSchema } from "./lib/schema";
...
export interface PlannerCoreProps {
  planType?: PlanType;
  initialPlan?: PlanExport;
  /** Override the full planner configuration. Defaults to outdoorConfig / indoorConfig based on planType. */
  config?: PlannerConfig;
  /** Called (debounced 2 s) whenever state changes. App handles API persistence. */
  onSave?: (plan: PlanExport) => Promise<void>;
  /** Called when user requests an export. App can check auth and show a modal before calling `execute`. */
  onRequestExport?: (kind: ExportKind, execute: () => void) => void;
  /** Render prop: receives dispatch so app can inject auth-aware upload controls. */
  uploadSlot?: (dispatch: React.Dispatch<PlannerAction>) => React.ReactNode;
}
...
export function PlannerCore({
  ...
  const resolvedConfig = config ?? (planType === "indoor" ? indoorConfig : outdoorConfig);
  ...
  // Load an imported plan from localStorage (set by /tile-plan/import)
  useEffect(() => {
  ...
  // Debounced save — calls onSave prop; no direct API knowledge in this component
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      const plan: PlanExport = buildPlanExport();
  ...
  function buildPlanExport(): PlanExport {
    return {
      version: 1,
  ...
  function requestExport(kind: ExportKind, execute: () => void) {
    if (onRequestExport) {
      onRequestExport(kind, execute);
    } else {
      execute();
    }
  }
  ...
  function handleExportPdf() {
    requestExport("pdf", () => exportPdf(state));
  }
  ...
  const isSnapped = useMemo(() => {
    ...
    if (Math.abs(by - ay) / len > 0.01) return false;
    const edgeMidY = (ay + by) / 2;
    let cy = 0;
    for (const [, vy] of verts) cy += vy;
    return edgeMidY > cy / n;
  }, [state.vertices]);
  ...
  return (
    <div className="flex h-full flex-col md:flex-row">
      ...
      <PlannerSidebar
        ...
        uploadNode={uploadSlot?.(dispatch)}
      />
    </div>
  );
}
