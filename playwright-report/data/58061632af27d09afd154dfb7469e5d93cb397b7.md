...
- Name: auth/register-homeowner.spec.ts >> Homeowner registration flow @smoke >> step 1 – submit email redirects to verify
- Location: e2e/auth/register-homeowner.spec.ts:23:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('button', { name: /continue|next|send/i }) resolved to 2 elements:
...
Call log:
  - waiting for getByRole('button', { name: /continue|next|send/i })
