...
- Name: auth/register-contractor.spec.ts >> Contractor registration flow >> step 3 – select contractor role redirects to password
- Location: e2e/auth/register-contractor.spec.ts:45:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByText(/contractor/i) resolved to 7 elements:
    1) <a href="/contractors" class="text-hint font-medium text-muted transition hover:text-ink">Find contractors</a> aka getByRole('link', { name: 'Find contractors' })
    2) <p class="text-body font-semibold text-ink">Contractor</p> aka getByRole('button', { name: 'Contractor I offer tiling,' })
    3) <p class="text-hint text-muted">Plan your garden, estimate materials, connect wit…</p> aka getByText('Plan your garden, estimate')
    4) <a href="/contractors" class="text-hint text-muted transition hover:text-ink">Find a contractor</a> aka getByRole('link', { name: 'Find a contractor' })
    5) <p class="mb-4 text-eyebrow text-muted/60">For contractors</p> aka getByText('For contractors')
    6) <a href="/register" class="text-hint text-muted transition hover:text-ink">Join as a contractor</a> aka getByRole('link', { name: 'Join as a contractor' })
    7) <a href="/contractors" class="text-hint text-muted transition hover:text-ink">Contractor directory</a> aka getByRole('link', { name: 'Contractor directory' })

Call log:
  - waiting for getByText(/contractor/i)
